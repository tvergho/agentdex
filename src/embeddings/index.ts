...
// ============ Yield for Read Requests ============
// Allows embed process to temporarily yield to readers
 ...
 * Returns true if another process is waiting to read the database.
 */
export function isReadRequestPending(): boolean {
  ...
  try {
    ...
    // Check if the requester is still running
    try {
      process.kill(lockData.pid, 0);
      return true;
    } catch {
      // Requester is dead, clean up stale lock
      try { unlinkSync(lockPath); } catch { /* ignore */ }
      return false;
    }
  } catch {
...
/**
 * Yield to pending readers.
 * Releases the embed lock temporarily, waits for reader to finish, then reacquires.
 * Returns true if yield happened, false if no reader was waiting.
 */
export async function yieldToReaders(yieldDurationMs = 2000): Promise<boolean> {
  if (!isReadRequestPending()) {
    return false;
  }

  // Release the lock to let the reader proceed
  releaseEmbedLock();

  // Wait for the reader to finish (or timeout)
  await new Promise(resolve => setTimeout(resolve, yieldDurationMs));

  // Reacquire the lock
  // Note: This might fail if another embed process started, but that's unlikely
  const reacquired = acquireEmbedLock();
  if (!reacquired) {
    // Another embed process took the lock - this shouldn't happen in normal use
